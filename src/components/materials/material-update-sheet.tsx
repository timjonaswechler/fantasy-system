...
// Formularvalidierungsschema (identisch mit MaterialCreateSheet)
...
export function MaterialUpdateSheet({
  ...
  // Formular mit Material-Daten aktualisieren, wenn Material geladen wird
  useEffect(() => {
    if (material) {
      form.reset({
        name: material.name,
        ...
        impactStrainAtYield: material.impactStrainAtYield,
        shearYield: material.shearYield,
        shearFracture: material.shearFracture,
        shearStrainAtYield: material.shearStrainAtYield,
        ...
        isStone: material.isStone,
        isGem: material.isGem,
        isOrganic: material.isOrganic,
        isFabric: material.isFabric,
      });

      // Zusätzliche Eigenschaften laden
      if (material.additionalProperties) {
        setAdditionalProps(
          Object.entries(material.additionalProperties).reduce<
            Record<string, string>
          >((acc, [key, value]) => {
  ...
  const onSubmit = async (values: MaterialFormValues) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Material erfolgreich aktualisiert");
  ...
  const handleCategoryChange = (value: string) => {
    ...
    // Flags automatisch anpassen - nur wenn sie nicht bereits manuell gesetzt wurden
    switch (category) {
      case MaterialCategory.METAL:
        if (
          !form.getValues("isMetal") &&
          !form.getValues("isStone") &&
          !form.getValues("isGem") &&
          !form.getValues("isOrganic") &&
          !form.getValues("isFabric")
        ) {
  ...
  return (
    <Sheet {...props}>
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Material bearbeiten</SheetTitle>
          <SheetDescription>
            Ändern Sie die Eigenschaften von "{material.name}"
          </SheetDescription>
        </SheetHeader>
        ...
        <Form {...form}>
          ...
          >
            {/* Formularfelder identisch mit MaterialCreateSheet */}
            <div className="space-y-4">
              ...
              {/* Rest des Formulars identisch mit MaterialCreateSheet */}
              {/* ... */}
