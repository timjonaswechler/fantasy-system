...
import { IMaterial } from "@/types/material"; // Import für das IMaterial Interface hinzugefügt

// Hilfsfunktion, um Material-Objekte mit minimalen Infos in vollständige IMaterial-Objekte zu konvertieren
function toIMaterial(partialMaterial: { id: any; name: any }): IMaterial {
  return {
    id: partialMaterial.id,
    name: partialMaterial.name,
    description: "", // Standardwerte für erforderliche Eigenschaften
    category: "",
    color: "",
    isMagical: false,
    rarity: "common",
...
export async function getTransformations(): Promise<MaterialTransformation[]> {
  try {
    ...
    return transformations.map((t) => ({
      id: t.id.toString(),
      sourceMaterial: toIMaterial({
        id: t.sourceMaterialId,
        name: t.sourceMaterialName,
      }),
      targetMaterial: toIMaterial({
